...
async def create_upload_file(file: UploadFile):
    # print(f"А вот и имя файла {file.filename}")
    image_array = np.frombuffer(file.file.read(), np.uint8)  # получаем numpy array байтов
    image = cv2.imdecode(image_array, cv2.IMREAD_COLOR)  # переводим из байтового формата в BGR
