...
# Теперь импортируем константы
from file_paths import css_files_fronted, IMAGE_ICON_FRONTEND, BASE_EXAMPLE_DIR_FRONTEND
...
def inject_custom_css():
    ...
    css_combined = ""
    for css_file in css_files_fronted:
...
def navigation_menu():
    image = Image.open(IMAGE_ICON_FRONTEND)  # для примера
...
def show_main_page():
    ...
    st.markdown("---")
    with open("../model_type.txt", "w", encoding="utf-8") as model_file:
        model_file.write(st.session_state.selected_model)
    uploading_detecting()
...
def show_info_page():
    ...
    <div class='info-card'>
        <p style='color: #cbd5e1; font-size: 15px; line-height: 1.7; margin: 0;'>
            ...
            Основное отличие у них во времени инференса, YOLO11n-obb работает немного побыстрее.
    ...
    <div class='info-card'>
        <p style='color: #cbd5e1; font-size: 15px; line-height: 1.7; margin: 0;'>
            ...
            В примерах переломов однако будут не все классы, так как не все представлены в тестовой выборке
...
def show_examples_page():
    ...
    DIR_EXAMPLES_GT = f"{BASE_EXAMPLE_DIR_FRONTEND}/{dir_gt}"
    DIR_EXAMPLES_PRED = f"{BASE_EXAMPLE_DIR_FRONTEND}/{dir_pred}"
    col1, col2 = st.columns(2)
